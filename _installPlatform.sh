...
TQA_NEXUS_USER=${TQA_NEXUS_USER:?Mandatory}
TQA_NEXUS_PASSWORD=${TQA_NEXUS_PASSWORD:?Mandatory}
...
docker run -ti -v ${PLF_INSTALL_VOLUME}:/srv -v ${DOWNLOADS_VOLUME}:/downloads -e OFFLINE=${OFFLINE} -e VERSION=${PLF_VERSION} -e REPOSITORY_USERNAME=${TQA_NEXUS_USER} -e REPOSITORY_PASSWORD=${TQA_NEXUS_PASSWORD} -e PLF_ARTIFACT_GROUPID=${PLF_ARTIFACT_GROUPID} -e PLF_ARTIFACT_ARTIFACTID=${PLF_ARTIFACT_ARTIFACTID} --name ${PLF_INSTALL_VOLUME}_installer ${PLF_INSTALLER_IMAGE} 1>&2
